...
(defn isbn-13? [s]
  (let [s (clojure.string/replace s #"-", "")]
    (if (re-find #"^\d{13}$" s)
      (= 0 (mod (isbn-verifier/isbn-sum s weights-13) 10))
      false)))

(defn isbn-10->isbn-13 [isbn-10]
  (let [isbn-10 (clojure.string/replace isbn-10 #"-" "")
        isbn-13 (str "978" (subs isbn-10 0 9))
        isbn-sum (isbn-verifier/isbn-sum isbn-13 weights-13)
...
(defn generate-isbn-10 []
  (let [random-9 (->> #(rand-int 10)
        ...
        isbn-sum (isbn-verifier/isbn-sum random-9 check-weights)
